...
我们现在的“编辑区 ↔ 预览区”同步可以拆成三个要点：

1. 谁在主导？
监听编辑区和预览区的 scroll 事件，每次滚动都会把 markdownScrollSyncState.lastScrollLeader 记成 "editor" 或 "preview"。这相当于告诉系统，“当前是谁在拖动”。

2. 主导方怎么驱动另一侧？
编辑区主导：利用 CodeMirror 的真实行数、标题锚点、滚动比例，算出预览容器应到达的位置，并在差值大于 3px 时才写入 scrollTop（避免抖动）。
预览区主导：把预览滚动的比例映射到编辑区，同样在超出 0.5px 时才更新。因为我们明白主导方是谁，就不会出现“刚滚过去又被拉回来”的情况。
3. “定时释放主导权”是怎么回事？
预览区滚动时还会启动一个 setTimeout（大概 180ms）。只要你继续滚动，这个计时器就会被重置；只有当你停止滚动且超时，主导权才被清空（lastScrollLeader 置空）。这样做的好处是防止刚停止的一瞬间，编辑区还没来得及反应就抢回主导权，从而产生反向拖拽或触底的体验。通俗讲：预览区滚动只要在“定时器”还没结束，就保持主导地位；时间到、没人继续滚，它才自动回到默认状态。

综上，整个同步是通过“记住谁是主导、分支处理跟随策略、靠阈值和定时器防抖”来实现的，既能做到无抖动、不卡顿，也能保证滚动方向的自然感。