...
AI_PROMPTS = {
    "script": {
        ...
        "template": (
            "你是一个幻灯片演讲稿写作专家。请根据下方LaTeX Beamer页面内容(每一个%后面都是给你的一些指示），"
            "优化对应的讲稿，使其表达更清晰、逻辑更流畅、适合演讲，内容不要脱离幻灯片主题。\n"
            "LaTeX页面内容如下：\n{page_tex}\n\n"
            "原始讲稿如下：\n{content}\n\n"
            "请直接返回优化后的讲稿文本，不要包含任何LaTeX代码或无关标记。"
        ),
    },
    "note": {
